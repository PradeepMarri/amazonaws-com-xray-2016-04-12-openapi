...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_UntagResource",
		mcp.WithDescription("Removes tags from an Amazon Web Services X-Ray group or sampling rule. You cannot edit or delete system tags (those with an <code>aws:</code> prefix)."),
		...
		mcp.WithArray("TagKeys", mcp.Required(), mcp.Description("Input parameter: Keys for one or more tags that you want to remove from an X-Ray group or sampling rule.")),
	)
