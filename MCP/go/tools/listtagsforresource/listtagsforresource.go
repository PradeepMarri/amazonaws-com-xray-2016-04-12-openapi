...
func ListtagsforresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/ListTagsForResource%s", cfg.BaseURL, queryString)
...
func CreateListtagsforresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_ListTagsForResource",
		mcp.WithDescription("Returns a list of tags that are applied to the specified Amazon Web Services X-Ray group or sampling rule."),
		mcp.WithString("NextToken", mcp.Description("Pagination token")),
		mcp.WithString("NextToken", mcp.Description("Input parameter: A pagination token. If multiple pages of results are returned, use the <code>NextToken</code> value returned with the current page of results as the value of this parameter to get the next page of results.")),
